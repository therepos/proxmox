...
_draw_progress() {
  ...
  printf "${BOLD}${PROGRESS_LABEL}${RESET}  ${PURPLE}${bar}${RESET}  ${pct}%% (${PROGRESS_CURRENT}/${PROGRESS_TOTAL})\n"
...
init_progress() {
  ...
  printf "${BOLD}${PROGRESS_LABEL}${RESET}  ${PURPLE}░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░${RESET}  0%% (0/${PROGRESS_TOTAL})\n"
...
# ── Step 1a: Count total JPEGs for scan progress ──────────────────────────────
info "Counting files to scan..."
TOTAL_JPGS=$(find "$ORIGINALS_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" \) | wc -l)
TOTAL_MOVS=$(find "$ORIGINALS_DIR" -type f -iname "*.mov" | wc -l)
success "Found ${TOTAL_JPGS} JPEG(s) and ${TOTAL_MOVS} MOV(s) to scan"
echo ""

# ── Step 1b: Scan Samsung Motion Photos ──────────────────────────────────────
info "Scanning for Samsung Motion Photos..."
declare -a SAMSUNG_PHOTOS=()

if [[ "$TOTAL_JPGS" -gt 0 ]]; then
  init_progress "$TOTAL_JPGS" "SCANNING"
...
if [[ "$TOTAL_MOVS" -gt 0 ]]; then
  ...
  while IFS= read -r -d '' mov; do
    ...
    tick_progress "$(basename "$mov")"
  done < <(find "$ORIGINALS_DIR" -type f -iname "*.mov" -print0)
  finish_progress "scan complete"
fi

APPLE_COUNT="${#APPLE_MOVS[@]}"
success "Apple Live Photos identified: ${APPLE_COUNT}"
echo ""
...
if [[ "$HAS_HEIC" = "1" ]]; then
  info "HEIC files detected — installing conversion tools..."
  ensure_cmd convert imagemagick && HAS_CONVERT=1 || true
  if [[ "$HAS_CONVERT" = "0" ]]; then
    ensure_cmd ffmpeg ffmpeg && HAS_FFMPEG=1 || true
  fi
  echo ""
fi
...
if [[ "$SAMSUNG_COUNT" -gt 0 ]]; then
  ...
  init_progress "$SAMSUNG_COUNT" "PROGRESS"
  ...
  finish_progress "samsung conversion done"
fi
...
if [[ "$APPLE_COUNT" -gt 0 ]]; then
  ...
  init_progress "$APPLE_COUNT" "PROGRESS"
  ...
  finish_progress "apple conversion done"
fi
