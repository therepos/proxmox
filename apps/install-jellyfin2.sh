...
if [[ -d "$CONFIG_DIR" && "$(ls -A $CONFIG_DIR)" ]]; then
    echo "Existing Jellyfin configuration found."
    read -p "Do you want to reset the setup? (y/n): " RESET_CONFIG
    if [[ "$RESET_CONFIG" == "y" || "$RESET_CONFIG" == "Y" ]]; then
        echo "Clearing configuration to trigger setup wizard..."
        rm -rf "$CONFIG_DIR/*" && status_message "success" "Configuration reset."
    else
        status_message "success" "Keeping existing configuration."
    fi
else
    mkdir -p "$CONFIG_DIR" && status_message "success" "Configuration directory created."
fi

# Ensure cache directory exists
if [[ ! -d "$CACHE_DIR" ]]; then
    mkdir -p "$CACHE_DIR" && status_message "success" "Cache directory created."
fi

# Ensure media directory exists
if [[ ! -d "$MEDIA_DIR" ]]; then
    mkdir -p "$MEDIA_DIR" && status_message "success" "Media directory created."
fi
