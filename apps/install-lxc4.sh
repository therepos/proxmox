...
# Enter the container to create a user and configure passwordless login
echo "Entering the container to create a user and configure passwordless login..."
sudo pct enter $CONTAINER_ID <<EOF
    # Check if root user exists, if not, create a new user
    if ! id "root" &>/dev/null; then
        useradd -m -s /bin/bash root
        passwd -d root
    fi
    
    # Create a new user (e.g., 'admin') if needed
    useradd -m -s /bin/bash admin
    passwd -d admin  # Disable password for the new user

    # Ensure SSH allows passwordless login by disabling PasswordAuthentication
