...
IS_WIN = os.name == 'nt'
OFFICE_TIMEOUT = 180          # seconds per Word/PowerPoint conversion
...
class Office:
    ...
    def _kill(self):
        ...
        for exe in ('WINWORD.EXE', 'POWERPNT.EXE'):
...
def main():
    ...
    office = Office(); t0 = time.time(); last = 0; n = 0; nerr = 0; nempty = 0
    try:
        for did, path, ext, name, doctype in todo:
            n += 1; ts = time.time(); local = os.path.join(tmpdir, f'{did}{ext}')
            status, err, parts = 'done', None, []
            try:
                shutil.copyfile(longpath(path), local)
                parts = extract(local, ext, tmpdir, office)
            ...
            db.execute('update docs set status=?, parts=?, chars=?, error=?, seconds=?, done_at=? where id=?',
                       (status, len(parts), sum(len(c) for _, _, c in parts), err, round(time.time() - ts, 2), datetime.datetime.now().strftime('%Y-%m-%d %H:%M'), did))
            if n % 25 == 0: db.commit()
            if time.time() - last > 1:
