...
if [ ! -f "$DOCKER_CONFIG" ]; then
    run_silent sudo tee "$DOCKER_CONFIG" > /dev/null <<EOF
{
    "storage-driver": "zfs",
    "runtimes": {
        "nvidia": {
...
EOF
    ...
    run_silent sudo jq '. + {"storage-driver": "zfs", "runtimes": {"nvidia": {"path": "nvidia-container-runtime", "runtimeArgs": []}}}' "$DOCKER_CONFIG" > /tmp/daemon.json
    run_silent sudo mv /tmp/daemon.json "$DOCKER_CONFIG"
fi
