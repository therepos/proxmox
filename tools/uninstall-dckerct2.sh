...
# Remove Docker's container storage directory (specific to the container)
DOCKER_CONTAINER_DIR="/mnt/sec/apps/$CONTAINER_NAME"
read -p "Do you want to remove the storage directory for container '$CONTAINER_NAME' at '$DOCKER_CONTAINER_DIR'? (y/n): " REMOVE_DOCKER_FILES
if [[ "$REMOVE_DOCKER_FILES" == "y" || "$REMOVE_DOCKER_FILES" == "Y" ]]; then
    if [[ -d "$DOCKER_CONTAINER_DIR" ]]; then
        ...
        status_message "error" "Directory '$DOCKER_CONTAINER_DIR' not found."
    fi
fi
