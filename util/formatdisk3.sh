...
echo -e "${RESET}Available disks:${RESET}"
lsblk -d -o NAME,SIZE | grep -v "NAME" | nl
read -p "Select a disk by entering its number: " disk_choice
DISK=$(lsblk -d -o NAME | sed -n "${disk_choice}p" | awk '{print "/dev/" $1}')

if [ ! -e "$DISK" ]; then
    echo -e "${RED}${RESET} Selected disk does not exist."
    exit 1
fi
...
if [ "$action_choice" == "1" ]; then
    ...
    parted $DISK resizepart 1 100% > /dev/null 2>&1
    resize2fs ${DISK}1 > /dev/null 2>&1
    if [ $? -eq 0 ]; then
...
if [ "$fs_choice" == "1" ]; then
    ...
    parted $DISK mkpart primary ext4 0% 100% > /dev/null 2>&1
