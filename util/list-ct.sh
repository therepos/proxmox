...
qm list | awk 'NR > 1 {print $1}' | while read VMID; do
    ...
    if qm guest exec "$VMID" -- echo "Guest Agent OK" >/dev/null 2>&1; then
        ...
        # Parse OUT_DATA for the primary external IP (exclude loopback and internal addresses)
        IP=$(echo "$OUT_DATA" | awk '/inet / && $4 !~ /127\.0\.0\.1/ && $4 !~ /172\.30/ {print $4}' | cut -d/ -f1 | head -n 1)
        if [ -z "$IP" ]; then
            IP="No IP Assigned"
        fi
