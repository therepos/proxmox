...
if zfs list -t snapshot | grep -q "${ZFS_POOL}@"; then
    zfs list -H -o name -t snapshot | grep "${ZFS_POOL}@" | while read snapshot; do
        # Extract snapshot date and calculate age
...
# Step 4: Check if the snapshot exists before destroying it
snapshot_exists=$(zfs list -t snapshot | grep -c "${ZFS_POOL}@${SNAPSHOT_NAME}")
...
if [ "$snapshot_exists" -gt 0 ]; then
    ...
    log_message "Snapshot ${SNAPSHOT_NAME} does not exist, skipping destroy."
fi
