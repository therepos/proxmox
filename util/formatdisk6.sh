...
if [ "$fs_choice" == "1" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        # Add ZFS pool to Proxmox storage configuration
        if ! grep -q "zfspool: $pool_name" /etc/pve/storage.cfg; then
            echo -e "Adding ZFS pool $pool_name to Proxmox storage configuration..."
            echo "zfspool: $pool_name" >> /etc/pve/storage.cfg
            echo "    pool $pool_name" >> /etc/pve/storage.cfg
            echo "    content images,iso" >> /etc/pve/storage.cfg
            systemctl reload pvedaemon
            echo -e "${GREEN}${RESET} ZFS pool $pool_name successfully added to Proxmox storage."
        else
            echo -e "${GREEN}${RESET} ZFS pool $pool_name already exists in the Proxmox storage configuration."
        fi
    else
